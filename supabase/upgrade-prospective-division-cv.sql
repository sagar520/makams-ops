-- ============================================================
-- Makams Ops — UPGRADE: prospective division, two more
-- departments, and the MI#### CV number.
...
select 'Division + CV numbers ready. prospectives=' || (select count(*) from public.prospectives)
       || ', numbered=' || (select count(*) from public.prospectives where cv_no is not null)
       || ', next=' || (select 'MI' || lpad((last_value + case when is_called then 1 else 0 end)::text, 4, '0')
                          from public.prospective_cv_seq) as result;
