-- ============================================================
-- Makams Ops — UPGRADE: prospectives arrive with no current company
-- Employees are still stamped CRIL; prospectives are not, and any
-- already mirrored across are cleared.
-- Paste into the SQL Editor and Run. Safe to re-run.
-- Run AFTER upgrade-candidates-superset.sql.
-- ============================================================

do $$ begin
  if to_regprocedure('public.ensure_candidate(text,text,text,text,text,text,text)') is null then
    raise exception 'Run upgrade-candidates-superset.sql first — this upgrade builds on it.';
  end if;
end $$;
...
select 'Current company: ' || count(*) filter (where source = 'Employee' and current_company = 'CRIL')
       || ' employees stamped CRIL, '
       || count(*) filter (where source = 'Prospective' and current_company is null)
       || ' prospective rows left blank' as result
  from public.candidates;
